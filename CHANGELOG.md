...
- `onVisibleRangeChange` now measures the center-column window against the band between the pinned overlays instead of the full viewport width. Center columns rendered entirely beneath a left or right pinned column are no longer reported as visible. The render window (`onRenderRangeChange`) is unchanged.
